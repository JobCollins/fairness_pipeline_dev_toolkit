from __future__ import annotations

from typing import Any, Dict, List, Optional, Set

import numpy as np
import pandas as pd
from sklearn.base import BaseEstimator, TransformerMixin


class DisparateImpactRemover(BaseEstimator, TransformerMixin):
    """
    Quantile-based repair for continuous features to reduce group-level distribution gaps.

    Fit stores the pooled empirical distribution and per-group reference distributions
    from the training frame. Transform maps each value through the *fitted* group CDF
    into the pooled quantile function — not through ranks of the current batch — so a
    row repaired alone matches the same row repaired inside a larger transform batch.

    Groups with fewer than ``min_group_size`` rows at fit time are not repaired (no
    stable reference). Unseen groups at transform time are left unchanged.

    - Works per feature in `features`
    - Uses `sensitive` (single attribute for Phase 2; extendable later)
    - Blend: repaired = (1 - repair_level) * orig + repair_level * mapped
    """

    def __init__(
        self,
        sensitive: str,
        features: List[str],
        repair_level: float = 1.0,
        min_group_size: int = 20,
    ):
        self.sensitive = sensitive
        self.features = list(features)
        self.repair_level = float(repair_level)
        self.min_group_size = int(min_group_size)

        # fitted state
        self.groups_: Optional[pd.Index] = None
        self.pool_quantiles_: Dict[str, np.ndarray] = {}
        self.pool_values_: Dict[str, np.ndarray] = {}
        # feature -> group label -> sorted training values (reference CDF)
        self.group_values_: Dict[str, Dict[Any, np.ndarray]] = {}
        self.repairable_groups_: Dict[str, Set[Any]] = {}

    def _empirical_quantiles(self, x: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """
        Return (q, v) such that q in [0,1] (monotone) and v are corresponding sorted values.
        """
        x_sorted = np.sort(x.astype(float))
        n = len(x_sorted)
        if n == 0:
            return np.array([0.0, 1.0]), np.array([0.0, 0.0])
        q = (np.arange(1, n + 1) - 0.5) / n
        return q, x_sorted

    def fit(self, X: pd.DataFrame, y: Optional[np.ndarray] = None) -> "DisparateImpactRemover":
        if self.sensitive not in X.columns:
            raise ValueError(f"Sensitive attribute '{self.sensitive}' not in DataFrame.")
        for f in self.features:
            if f not in X.columns:
                raise ValueError(f"Feature '{f}' not in DataFrame.")

        self.groups_ = X[self.sensitive].astype("category").cat.categories
        self.group_values_ = {}
        self.repairable_groups_ = {}

        # compute pooled quantiles per feature
        for f in self.features:
            q, v = self._empirical_quantiles(X[f].dropna().to_numpy())
            self.pool_quantiles_[f] = q
            self.pool_values_[f] = v

            by_group: Dict[Any, np.ndarray] = {}
            repairable: Set[Any] = set()
            g_col = X[self.sensitive]
            for cat in pd.unique(g_col.to_numpy()):
                vals = X.loc[g_col == cat, f].dropna().to_numpy(dtype=float)
                if vals.size < self.min_group_size:
                    continue
                by_group[cat] = np.sort(vals)
                repairable.add(cat)
            self.group_values_[f] = by_group
            self.repairable_groups_[f] = repairable
        return self

    def _interp(self, q_grid: np.ndarray, v_grid: np.ndarray, q: np.ndarray) -> np.ndarray:
        # piecewise linear interpolation of quantiles into values
        if len(q_grid) == 0:
            return np.full_like(q, np.nan, dtype=float)
        return np.interp(q, q_grid, v_grid, left=v_grid[0], right=v_grid[-1])

    def _value_to_quantile(self, values: np.ndarray, ref_sorted: np.ndarray) -> np.ndarray:
        """Map feature values to [0, 1] quantiles via the fitted group reference CDF."""
        n = len(ref_sorted)
        if n == 0:
            return np.full(len(values), 0.5, dtype=float)
        q_grid = (np.arange(1, n + 1) - 0.5) / n
        return np.interp(values, ref_sorted, q_grid, left=0.0, right=1.0)

    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        if self.groups_ is None:
            raise RuntimeError("DisparateImpactRemover must be fitted before transform.")

        df = X.copy()
        g = df[self.sensitive]

        for f in self.features:
            pool_q = self.pool_quantiles_[f]
            pool_v = self.pool_values_[f]
            group_refs = self.group_values_[f]
            repairable = self.repairable_groups_[f]
            x = df[f].to_numpy(dtype=float, copy=True)

            for cat in pd.unique(g.to_numpy()):
                if cat not in repairable:
                    continue
                idx = np.where(g.to_numpy() == cat)[0]
                if idx.size == 0:
                    continue

                vals = x[idx]
                q = self._value_to_quantile(vals, group_refs[cat])
                mapped = self._interp(pool_q, pool_v, q)
                repaired = (1.0 - self.repair_level) * vals + self.repair_level * mapped
                x[idx] = repaired

            df[f] = x

        return df
